...
<a href="#search-overlay" class="sr-only focus:not-sr-only focus:absolute focus:top-2 focus:left-2 bg-indigo-700 text-white px-4 py-2 rounded z-50">{{ __('Skip to search') }}</a>
<div
    x-data="{
        show: false,
        query: '',
...
>
    ...
    >
        <div class="overflow-hidden rounded-xl bg-white shadow-2xl ring-1 ring-black/5 dark:bg-gray-900 dark:ring-white/10">
            ...
            <h2 id="search-overlay-title" class="sr-only">{{ __('Search') }}</h2>
            <!-- Close Button -->
            <button type="button" @click="show = false" class="absolute right-4 top-4 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 bg-white dark:bg-gray-900 rounded-full p-2 z-10" aria-label="{{ __('Close search') }}">
            ...
            <!-- Search Input -->
            <div class="relative">
                <label for="search-input" class="sr-only">{{ __('Search articles') }}</label>
            ...
            <div
                ...
                class="border-t border-gray-100 px-4 py-6 text-center text-sm dark:border-gray-800"
            >
                <p class="text-gray-500 dark:text-gray-400">{{ __('No results found for') }} "<span x-text="query"></span>"</p>
            </div>
            ...
            <div class="flex items-center justify-between border-t border-gray-100 px-4 py-2 dark:border-gray-800">
                <p class="text-xs text-gray-500 dark:text-gray-400">
                    {{ __('Press') }} <kbd class="font-sans">↑</kbd> <kbd class="font-sans">↓</kbd> {{ __('to navigate,') }}
                    <kbd class="font-sans">Enter</kbd> {{ __('to select,') }}
                    <kbd class="font-sans">Esc</kbd> {{ __('to close') }}
                </p>
            </div>
        </div>
    </div>
